...
$listOrganizations = function (Request $request, Response $response) use ($db): Response {
    $stmt = $db->query('SELECT id, name, address_1, address_2, city, state_province, postal_code, country FROM tt_organization ORDER BY name');
...
$getOrganization = function (Request $request, Response $response, array $args) use ($db): Response {
    $stmt = $db->prepare('SELECT id, name, address_1, address_2, city, state_province, postal_code, country FROM tt_organization WHERE id = ?');
...
$createOrganization = function (Request $request, Response $response) use ($db): Response {
    ...
    $stmt = $db->prepare(
        'INSERT INTO tt_organization (name, address_1, address_2, city, state_province, postal_code, country)
         VALUES (?, ?, ?, ?, ?, ?, ?)'
    );
    $stmt->execute([
        $data['name'],
...
$updateOrganization = function (Request $request, Response $response, array $args) use ($db): Response {
    ...
    foreach (['name', 'address_1', 'address_2', 'city', 'state_province', 'postal_code', 'country'] as $field) {
