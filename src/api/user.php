...
$listUsers = function (Request $request, Response $response) use ($db): Response {
	...
	$cols = 'u.id, u.username, u.email_address, u.first_name, u.last_name, u.country, u.confirmed, u.is_admin, u.is_superuser, u.registered, u.last_login';

	if ($perms['superuser']) {
		$stmt = $db->query("SELECT $cols FROM tt_user u ORDER BY u.last_name, u.first_name");
		$users = $stmt->fetchAll();
		...
		$stmt = $db->prepare(
			"SELECT DISTINCT $cols
			 FROM tt_user u
			 JOIN tt_organization_user_map m ON m.user_id = u.id
			 WHERE m.organization_id IN ($ph)
...
$getUser = function (Request $request, Response $response, array $args) use ($db): Response {
	...
	if (!testUserIsntTargetInOrg($db, $perms, $currentUserId, $targetId)) return unauthorized($response);
...
$updateUser = function (Request $request, Response $response, array $args) use ($db): Response {
	...
	if (array_key_exists('organization_ids', $data)) {
		$db->prepare('DELETE FROM tt_organization_user_map WHERE user_id = ?')->execute([$targetId]);
		foreach ((array)$data['organization_ids'] as $orgId) {
			$orgId = (int)$orgId;
			if ($orgId > 0) {
				$db->prepare('INSERT INTO tt_organization_user_map (organization_id, user_id) VALUES (?, ?)')->execute([$orgId, $targetId]);
			}
		}
	}
...
$deleteUser = function (Request $request, Response $response, array $args) use ($db): Response {
	...
	$db->prepare('DELETE FROM tt_user WHERE id = ?')->execute([$targetId]);
